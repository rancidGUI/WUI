...
def add_cronjob(data):
    if data['cron_job'][0]['Type'] == "tftp":
        ...
        cmd ="/opt/rancid/bin/rancid-run"+tab_param[data['cron_job'][0]['Type']]+" "+data['cron_job'][0]['Name']
...
def create_cronjob(data):
    ...
    cmd1 += data['cron_job'][0]['DOWEEK']+" "
    cmd1 += data['cron_job'][0]['Commands']
    if data['cron_job'][0]['Comments'] is not None:
        cmd1 += " # "+data['cron_job'][0]['Comments']
    return cmd1
...
def tftp(data):
    ...
    excecute_cmd("rm -f ./tftp_log.txt")
    return "ok"
